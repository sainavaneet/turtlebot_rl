...
class TurtlebotController:
    ...
    def convert_rl_action_to_velocity(self, action):
        proximity_threshold = 0.2  # define your own threshold
        min_linear_vel, max_linear_vel = 0.0, 0.4
        min_angular_vel, max_angular_vel = 0.0, 0.8

        if self.obstacle_distance < proximity_threshold:
            # You can refine these rules based on the behavior you observe in the robot.
            adjusted_linear_velocity = max_linear_vel * (self.obstacle_distance / proximity_threshold)
            adjusted_angular_velocity = max_angular_vel  # Max turning speed as an example
        else:
            linear_velocity_range = max_linear_vel - min_linear_vel
            angular_velocity_range = max_angular_vel - min_angular_vel

            adjusted_linear_velocity = min_linear_vel + linear_velocity_range * action[0]
            adjusted_angular_velocity = min_angular_vel + angular_velocity_range * action[1]
    ...
    def calculate_mpc_control_input(self):
        ...
        if result.success:
            optimal_control = result.x[:u_dim]
            return optimal_control
        else:
            return None

    def get_reward(self):
        if self.obstacle_distance <= self.collision_distance:
    ...
    def run(self):
        while not rospy.is_shutdown():
            if self.obstacle_distance <= self.collision_distance:
                ...
                last_action = None
                last_state = None
                self.optimize_policy(last_state, last_action, reward, current_state)
